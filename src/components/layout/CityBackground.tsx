...
interface TrafficLight {
  group: THREE.Group;
  redLens: THREE.Mesh<THREE.SphereGeometry, THREE.MeshBasicMaterial>;
  yellowLens: THREE.Mesh<THREE.SphereGeometry, THREE.MeshBasicMaterial>;
  greenLens: THREE.Mesh<THREE.SphereGeometry, THREE.MeshBasicMaterial>;
  ptLight: THREE.PointLight;
  isNS: boolean;
}

export default function CityBackground({ theme = 'dark' }: { theme?: 'light' | 'dark' }) {
  ...
  const vehicleLightsRef = useRef<THREE.PointLight[]>([]);
  const billboardsRef = useRef<THREE.Mesh<THREE.PlaneGeometry, THREE.MeshBasicMaterial>[]>([]);
  ...
  useEffect(() => {
    ...
    sceneRef.current.background = new THREE.Color(skyHexColor);
    ...
    if (dirLightRef.current) {
      ...
      dirLightRef.current.color.setHex(isLight ? 0xfffaf0 : 0x3e185e);
    }
    ...
    // Update vehicle light projections
    vehicleLightsRef.current.forEach((l) => {
      l.intensity = isLight ? 0.05 : (l.userData['originalIntensity'] as number || 1.2);
    });

    // Re-draw billboards to match day/night style
    billboardsRef.current.forEach((bb) => {
      const text = bb.userData['text'] as string || 'NEXO PAY';
      const sub = bb.userData['sub'] as string || '';
      const bCanvas = makeBillboardCanvas(text, sub, 512, 256, isLight);
  ...
  useEffect(() => {
    ...
    // Neon lights
    const neonColors = [0x6366f1, 0x0ea5e9, 0x10b981, 0xf43f5e];
    const neonLights: THREE.PointLight[] = [];
    for (let i = 0; i < 4; i++) {
      const color = neonColors[i] || 0x6366f1;
    ...
    for (let x = -xGrid; x <= xGrid; x += 3) {
      for (let z = -zGrid; z <= zGrid; z += 3) {
        ...
        for (let f = 0; f < faceCount; f++) {
          const winColor = neonColors[Math.floor(Math.random() * neonColors.length)] || 0x6366f1;
    ...
    const tempBillboards: THREE.Mesh<THREE.PlaneGeometry, THREE.MeshBasicMaterial>[] = [];
    ...
    const tempVehicleLights: THREE.PointLight[] = [];
    ...
    const animate = () => {
      ...
      neonLights.forEach((light, i) => {
        ...
        const isSceneL = sceneRef.current && sceneRef.current.background instanceof THREE.Color && sceneRef.current.background.r > 0.5;
        light.intensity = (isSceneL ? 0.2 : 1.2) + Math.sin(elapsed * 2 + i) * (isSceneL ? 0.05 : 0.4);
      });
      ...
      buildingGroup.children.forEach((mesh, idx) => {
        if (mesh instanceof THREE.Mesh && mesh.material instanceof THREE.MeshBasicMaterial) {
          ...
          const isSceneL = sceneRef.current && sceneRef.current.background instanceof THREE.Color && sceneRef.current.background.r > 0.5;
          mesh.material.opacity = (isSceneL ? 0.15 : 0.4) + Math.abs(wave) * (isSceneL ? 0.15 : 0.4);
        }
      });
    ...
    return () => {
      ...
      vehicles.forEach((veh) => {
        veh.mesh.geometry.dispose();
        if (Array.isArray(veh.mesh.material)) {
          veh.mesh.material.forEach((m) => m.dispose());
        } else {
          veh.mesh.material.dispose();
        }
        veh.mesh.children.forEach((c) => {
          c.children.forEach((gc) => {
            if (gc instanceof THREE.Mesh) {
              gc.geometry.dispose();
              if (Array.isArray(gc.material)) {
                gc.material.forEach((m) => m.dispose());
              } else {
                gc.material.dispose();
              }
            }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- theme changes are handled by the dedicated effect above (lines ~112-156); the scene setup runs only once on mount
  }, []);
